...
const Chat = ({ currentUser }) => {
  ...
  const handleCreateGroupChat = async () => {
    const targetClientId = isAdmin
      ? selectedClientForNewChat?.id
      : currentUser.client_id;
    const targetClientName = isAdmin
      ? selectedClientForNewChat?.nombre
      : "Mi Sucursal";
  ...
  const getRoomDisplayName = (room) => {
    if (room.type === "GROUP")
      return room.name || room.clientes?.nombre || "Chat de Sucursal";
    const other = room.chat_participants?.find(
        ...
        <div className="w-full h-full flex flex-col">
          <div className="p-4 md:p-5 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/30 shrink-0">
            ...
            <button
              ...
              className="p-2.5 bg-red-600 text-white rounded-xl hover:bg-red-700 transition-all shadow-lg shadow-red-600/20 active:scale-95 shrink-0"
            >
              <Plus size={20} />
            </button>
          </div>
              ...
              chatRooms.map((room) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-12 h-12 shrink-0 rounded-2xl flex items-center justify-center text-white font-bold shadow-sm ${room.type === "GROUP" ? "bg-blue-600" : "bg-slate-800 dark:bg-slate-700"}`}
                    >
                      {room.type === "GROUP" ? (
                        <Users size={20} />
                      ) : (
                    ...
                    <div className="flex-1 min-w-0 py-1">
                      <h3 className="text-[15px] font-bold text-slate-800 dark:text-slate-100 truncate leading-tight mb-1">
                        {getRoomDisplayName(room)}
                      </h3>
                      <div className="flex items-center gap-2">
                        <span
                          className={`text-[9px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wider shrink-0 ${room.type === "GROUP" ? "bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400" : "bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400"}`}
                        >
        ...
        <div className="w-full h-full flex flex-col bg-slate-50 dark:bg-slate-950/50">
          ...
          <div className="p-3 border-b border-slate-200 dark:border-slate-800 flex items-center gap-3 bg-white dark:bg-slate-900 shadow-sm z-10 shrink-0">
            <button
              ...
              className="p-2 -ml-1 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors shrink-0"
            >
              <ChevronLeft size={24} />
            </button>
            <div
              className={`w-10 h-10 shrink-0 rounded-xl flex items-center justify-center text-white ${selectedRoom.type === "GROUP" ? "bg-blue-600" : "bg-slate-800"}`}
            >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[9px] text-slate-400 font-medium uppercase tracking-tighter truncate">
              ...
              return (
                ...
                >
                  <div className={`max-w-[85%] group`}>
                    ...
                    <div
                      className={`p-3 rounded-2xl shadow-sm relative ${isMe ? "bg-red-600 text-white rounded-tr-none" : "bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 border border-slate-200 dark:border-slate-700 rounded-tl-none"}`}
                    >
                      <p className="text-[13px] leading-relaxed whitespace-pre-wrap break-words">
                        {msg.content}
                      </p>
                      <p
                        className={`text-[9px] mt-1.5 font-medium opacity-60 ${isMe ? "text-right" : "text-left"}`}
                      >
          ...
          <form
            ...
            className="p-3 pr-20 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex gap-2 items-center shrink-0"
          >
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[9000] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-3xl shadow-2xl p-6 text-center border border-slate-200 dark:border-slate-800">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
              Esta acción es irreversible y borrará todos los mensajes para
              todos los participantes.
            </p>
            <div className="flex flex-col gap-3">
              <button
                onClick={() => handleDeleteRoom(showDeleteConfirm)}
                className="w-full py-3.5 bg-red-600 text-white rounded-2xl font-bold hover:bg-red-700 transition-all"
              >
                Eliminar
              </button>
              <button
                onClick={() => setShowDeleteConfirm(null)}
                className="w-full py-3.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-2xl font-bold hover:bg-slate-200 dark:hover:bg-slate-700 transition-all"
              >
                Cancelar
              </button>
            </div>
            ...
            <div className="p-6 space-y-6">
              <div className="space-y-3">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setSelectedUserForNewChat(
                      usersForNewChat.find((u) => u.id === e.target.value),
                    )
                  }
                >
                ...
                <button
                  ...
                  disabled={!selectedUserForNewChat || isCreatingRoom}
                  className="w-full py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl font-bold text-sm hover:bg-slate-800 dark:hover:bg-slate-100 transition-all disabled:opacity-30 shadow-lg"
                >
              ...
              {isAdmin && (
                <div className="space-y-3 pt-4 border-t border-slate-100 dark:border-slate-800">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setSelectedClientForNewChat(
                        clientsForNewChat.find((c) => c.id === e.target.value),
                      )
                    }
                  >
                    <option value="">Seleccionar empresa/sucursal...</option>
                    {clientsForNewChat.map((c) => (
                  ...
                  <button
                    ...
                    disabled={!selectedClientForNewChat || isCreatingRoom}
                    className="w-full py-4 bg-blue-600 text-white rounded-2xl font-bold text-sm hover:bg-blue-700 transition-all disabled:opacity-30 shadow-lg shadow-blue-600/20"
                  >
