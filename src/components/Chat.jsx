...
const Chat = ({ currentUser }) => {
  ...
  const handleCreateGroupChat = async () => {
    ...
    try {
      ...
      if (users) {
        const participants = users.map(u => ({ room_id: newRoom.id, user_id: u.id }));
        await supabase.from('chat_participants').insert(participants);
      }

      toast.success('Chat grupal creado');
      setShowNewChatModal(false);
      fetchChatRooms();
    } catch (error) {
  ...
  const getRoomDisplayName = (room) => {
    if (room.type === 'GROUP') {
      ...
      const otherParticipant = room.chat_participants?.find(p => p.user_id !== currentUser.id);
      return otherParticipant?.profiles?.full_name || 'Chat Privado';
    }
  };

  return (
    <div className="flex w-full h-full bg-white dark:bg-slate-900 overflow-hidden font-sans relative rounded-lg">
      ...
      {!selectedRoom ? (
        <div className="w-full h-full flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto">
            {loadingRooms ? (
              <div className="p-12 text-center"><Loader2 className="animate-spin mx-auto text-red-600 mb-2" /> <span className="text-xs text-slate-400">Sincronizando...</span></div>
            ) : chatRooms.length === 0 ? (
        ...
        <div className="w-full h-full flex flex-col bg-slate-50 dark:bg-slate-950/50">
          ...
          {/* Input Form */}
          <form onSubmit={sendMessage} className="p-3 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex gap-2 items-center shrink-0">
