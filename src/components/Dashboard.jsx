...
export default function Dashboard({ navigateTo, stats }) {
  ...
  const menuItems = [
    { id: 'form', label: 'Nueva Inspección', desc: 'Protocolo NFPA', icon: Flame, color: 'bg-red-600' },
    { id: 'sites', label: 'Asset Radar', desc: 'Mapa en vivo', icon: MapIcon, count: stats?.totalAssets || 0, color: 'bg-slate-800 dark:bg-slate-700' },
    { id: 'list', label: 'Historial Técnico', desc: 'Reportes y PDF', icon: ClipboardList, count: stats?.totalReports || 0, color: 'bg-slate-800 dark:bg-slate-700' },
    { id: 'critical', label: 'Hallazgos Críticos', desc: 'Urgencias', icon: AlertOctagon, count: stats?.criticals || 0, color: 'bg-red-600' }
  ];
      ...
      <div className="space-y-4 pt-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Building2 className="text-slate-400 dark:text-slate-500" size={20} strokeWidth={1.5} />
        ...
        {loading ? (
            <div className="p-8 text-center text-slate-400 dark:text-slate-500 font-medium text-sm">Cargando empresas...</div>
        ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {clientes.length > 0 ? (clientes.map(cliente => (
                <button key={cliente.id} onClick={() => handleOpenDetails(cliente)} className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between group hover:shadow-md hover:border-slate-300 dark:hover:border-slate-600 transition-all text-left">
                  ...
                  <ChevronRight className="text-slate-300 dark:text-slate-600 group-hover:text-red-600 dark:group-hover:text-red-500 transition-colors" size={16} />
                </button>
              ))) : (
                <div className="p-8 text-center text-slate-400 dark:text-slate-500 font-medium text-sm col-span-full">No hay empresas registradas o no tienes acceso.</div>
              )}
            </div>
        )}
      </div>
          ...
          <div className="relative w-full max-w-sm bg-white dark:bg-slate-900 h-full shadow-xl animate-in slide-in-from-right duration-300 flex flex-col border-l border-slate-200 dark:border-slate-700">
            ...
            <div className="p-6 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 relative">
              ...
              <div className="flex items-center gap-4 mt-1">
                <div className="bg-red-50 dark:bg-red-900/20 p-3 rounded-xl text-red-600 dark:text-red-500 border border-red-100 dark:border-red-900/30"><Building2 size={24} strokeWidth={1.5}/></div>
                <div>
                  <span className="text-xs font-medium text-slate-500 dark:text-slate-400">Ficha Técnica</span>
                  <h2 className="text-lg font-semibold text-slate-900 dark:text-white mt-0.5 truncate max-w-[200px]">{selectedClient.nombre}</h2>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-700 pb-3">
                  <h4 className="text-xs font-medium text-slate-500 dark:text-slate-400 flex items-center gap-2"><MapPin size={14} className="text-red-500" strokeWidth={1.5}/> Planta</h4>
                  {isAdmin && (
                    <button onClick={() => setIsEditing(!isEditing)} className="text-red-600 dark:text-red-500 font-medium text-xs px-3 py-1 bg-red-50 dark:bg-red-900/20 rounded-md hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors border border-red-100 dark:border-red-900/30">
                ...
                <div className="space-y-3">
                  <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-lg border border-slate-100 dark:border-slate-700">
                    <label className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1.5">Dirección</label>
                    {isEditing ? (
                      <textarea className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-md p-3 text-sm text-slate-800 dark:text-slate-200 outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500" rows="2" value={editData.direccion || ''} onChange={e => setEditData({...editData, direccion: e.target.value})}/>
                    ) : (
                      <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">{selectedClient.direccion || 'Sin domicilio registrado'}</p>
                    )}
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-lg border border-slate-100 dark:border-slate-700">
                    <label className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1.5">Teléfono</label>
                    {isEditing ? (
                      <input type="text" className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-md p-3 text-sm text-slate-800 dark:text-slate-200 outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500" value={editData.telefono || ''} onChange={e => setEditData({...editData, telefono: e.target.value})}/>
                    ) : (
                      <p className="text-sm text-slate-700 dark:text-slate-300">{selectedClient.telefono || 'Sin teléfono'}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white dark:bg-slate-800 p-5 rounded-lg border border-slate-200 dark:border-slate-700 space-y-4">
                   <div>
                     <label className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1.5">Nombre del Jefe</label>
                     {isEditing ? (
                       <input className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-md p-3 text-sm text-slate-800 dark:text-slate-200 outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500" value={editData.encargado_nombre || ''} onChange={e => setEditData({...editData, encargado_nombre: e.target.value})}/>
                     ) : (
                       <p className="text-base font-semibold text-slate-900 dark:text-white">{selectedClient.encargado_nombre || 'Pendiente'}</p>
                     )}
                   </div>
                   
                   <div className="space-y-4">
                     <div>
                       <label className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1.5">Email de Acceso</label>
                       {isEditing ? (
                         <input className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-md p-3 text-sm text-slate-800 dark:text-slate-200 outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500" value={editData.encargado_email || ''} onChange={e => setEditData({...editData, encargado_email: e.target.value})}/>
                       ) : (
                         <p className="text-sm text-red-600 dark:text-red-500 font-medium truncate">{selectedClient.encargado_email || 'Sin vincular'}</p>
                       )}
                     </div>

                     {isEditing && (
                       <div className="animate-in slide-in-from-bottom-2 duration-300">
                         <label className="text-xs font-medium text-blue-600 dark:text-blue-400 block mb-1.5">Asignar Contraseña</label>
                         <div className="relative">
                           ...
                           />
                           <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300">
              ...
              <div className="bg-amber-50 dark:bg-amber-900/20 p-5 rounded-lg border border-amber-200 dark:border-amber-800">
                ...
                {isEditing ? (
                  <textarea className="w-full bg-white dark:bg-slate-900 border border-amber-200 dark:border-amber-800 rounded-md p-3 text-sm text-slate-800 dark:text-slate-200 h-32 outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500" value={editData.notas_internas || ''} onChange={e => setEditData({...editData, notas_internas: e.target.value})}/>
                ) : (
            ...
            <div className="p-5 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex gap-3">
              {isEditing ? (
                ...
                <>
                  <button onClick={() => navigateTo('list')} className="flex-1 bg-slate-900 dark:bg-white hover:bg-slate-800 dark:hover:bg-slate-100 text-white dark:text-slate-900 py-3 rounded-lg font-medium text-sm flex items-center justify-center gap-2 transition-colors shadow-sm"><ExternalLink size={16}/> Historial</button>
                  {isAdmin && (
                    <button onClick={() => handleDeleteClient(selectedClient.id, selectedClient.nombre)} className="p-3 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-500 rounded-lg hover:bg-red-600 hover:text-white dark:hover:bg-red-600 dark:hover:text-white transition-colors border border-red-100 dark:border-red-900/30"><Trash2 size={18}/></button>
                  )}
                </>
              )}
            </div>
