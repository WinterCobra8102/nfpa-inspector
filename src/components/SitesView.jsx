...
export default function SitesView({ navigateTo, currentUser }) {
  ...
  const loadSitesFromDB = async () => {
    try {
      ...
      const standardizedSites = allCompanies
        .map((c) => ({
          id: c.id,
          name: c.nombre,
          address: c.direccion,
      ...
      geocoder.geocode(
        ...
        async (results, status) => {
          if (status === "OK" && results[0]) {
            ...
            toast.error(
              "No se encontró esa dirección en el mapa. Sé más específico (ej. Calle 60 #123, Mérida).",
              { id: loadingToast },
            );
          }
        },
  ...
  if (loadError)
    return (
      <div className="h-full flex items-center justify-center bg-white dark:bg-slate-950 text-red-600 font-medium">
        Error al cargar Google Maps
      </div>
    );
  if (!isLoaded)
    return (
      <div className="h-full flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 text-slate-600 dark:text-slate-400">
  ...
  return (
    <div className="h-full w-full relative overflow-hidden bg-slate-50 dark:bg-slate-950">
      ...
      <div className="absolute top-5 left-1/2 -translate-x-1/2 w-[95%] md:w-[480px] z-[2000] space-y-2 pointer-events-none">
        ...
        <div
          className={`bg-white/95 dark:bg-slate-900/95 backdrop-blur-lg border border-slate-200 dark:border-slate-700 rounded-xl px-5 py-3 flex items-center gap-3 shadow-lg pointer-events-auto transition-opacity ${showManualForm ? "opacity-0 pointer-events-none" : "opacity-100"}`}
        >
          <Search
          ...
          <div className="flex-1">
            ...
            >
              <input
                ...
                placeholder="Buscar sucursal o plaza en el mapa..."
                className="bg-transparent border-none w-full text-slate-800 dark:text-slate-200 font-medium text-sm outline-none placeholder:text-slate-400 dark:placeholder:text-slate-500"
              />
            </Autocomplete>
          </div>
          {searchTerm && (
          ...
          >
            <div className="p-4 min-w-[240px] bg-white rounded-lg text-center">
              ...
              <button
                ...
                className="w-full bg-red-600 hover:bg-red-700 text-white py-2.5 rounded-lg font-medium text-sm shadow-sm active:scale-[0.98] transition-all flex items-center justify-center gap-2"
              >
        ...
        {activeSite && !showManualForm && (
          ...
          >
            <div className="p-4 text-center min-w-[200px] space-y-3">
              <div>
                <span
                  className={`text-xs font-medium px-2 py-0.5 rounded-md text-white ${activeSite.overallStatus === "CRÍTICO" ? "bg-red-500" : "bg-green-500"}`}
                >
                  {activeSite.overallStatus}
                </span>
                <h4 className="font-semibold text-sm text-slate-800 mt-2">
                  {activeSite.name}
                </h4>
      ...
      <div
        className={`absolute top-5 left-4 z-[1000] flex flex-col gap-3 transition-opacity ${showManualForm ? "opacity-0 pointer-events-none" : "opacity-100"}`}
      >
        <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-lg border border-slate-200 dark:border-slate-700 p-1.5 rounded-xl shadow-lg flex flex-col gap-1 w-fit">
          <button
            ...
            className="p-3 rounded-lg transition-all text-slate-400 dark:text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-700"
          >
            <LayoutGrid size={18} />
          </button>
        </div>
        ...
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-[3000] w-[90%] md:w-[400px] bg-white/95 dark:bg-slate-900/95 backdrop-blur-lg border border-slate-200 dark:border-slate-700 p-6 rounded-2xl shadow-2xl animate-in slide-in-from-bottom-10">
          ...
          <div className="space-y-4">
            <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium leading-relaxed">
              Escribe la{" "}
              <span className="font-bold text-red-500">calle exacta</span>. El
              sistema calculará las coordenadas de forma automática.
            </p>
            <div>
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                Nombre Sucursal <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
                autoFocus
                placeholder="Ej: Bodega TLETL Norte"
            ...
            <div>
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                Dirección para Buscar <span className="text-red-500">*</span>
              </label>
              <textarea
                rows="2"
            ...
            <div className="pt-2">
              <button
                ...
                className="w-full bg-red-600 hover:bg-red-700 text-white py-3 rounded-xl font-bold text-sm shadow-md shadow-red-600/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
              >
      ...
      {/* Botón GPS */}
      <div
        className={`absolute bottom-32 right-5 z-[1000] transition-opacity ${showManualForm ? "opacity-0 pointer-events-none" : "opacity-100"}`}
      >
        <button
          onClick={() => userPos && map?.panTo(userPos)}
          className="bg-red-600 hover:bg-red-700 p-4 rounded-xl border border-red-500/20 text-white active:scale-95 shadow-lg transition-all"
        >
          <LocateFixed size={22} />
        </button>
      </div>
      ...
      <div
        className={`absolute bottom-6 left-4 right-4 z-[1000] transition-opacity ${showManualForm ? "opacity-0 pointer-events-none" : "opacity-100"}`}
      >
        <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-lg border border-slate-200 dark:border-slate-700 p-4 rounded-xl shadow-lg max-w-lg mx-auto flex items-center justify-around">
          ...
          <div className="text-center">
            ...
            <div className="flex items-center gap-2 justify-center">
              ...
              <span className="text-2xl font-semibold text-red-600 dark:text-red-500">
                {
                  filteredSites.filter((s) => s.overallStatus === "CRÍTICO")
                    .length
                }
              </span>
            </div>
