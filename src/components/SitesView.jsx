    ...
    <div className="h-full w-full relative overflow-hidden bg-[#111]">
      ...
      <div className="absolute top-6 left-1/2 -translate-x-1/2 w-[95%] md:w-[500px] z-[2000]">
        <div className="bg-[#1a1a1a]/95 backdrop-blur-xl border border-white/10 rounded-3xl px-6 py-4 flex items-center gap-4 shadow-2xl">
          ...
          {searchTerm && <button onClick={() => { setSearchTerm(''); setSelectedPlace(null); }}><X className="text-slate-500 hover:text-white" size={18}/></button>}
        </div>
      </div>
        ...
        {selectedPlace && (
          <InfoWindow position={{ lat: selectedPlace.lat, lng: selectedPlace.lng }} onCloseClick={() => setSelectedPlace(null)}>
            <div className="p-5 min-w-[260px] bg-white rounded-2xl text-center">
              ...
              <button onClick={handleRegisterCompany} className="w-full bg-red-600 hover:bg-red-700 text-white py-3 rounded-xl font-black text-xs uppercase shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2">
          ...
          <InfoWindow position={{ lat: activeSite.lat, lng: activeSite.lng }} onCloseClick={() => setActiveSite(null)}>
            <div className="p-3 text-center min-w-[200px] space-y-3">
              ...
              <div className="space-y-1.5 pt-2 border-t border-slate-100">
                <button className="w-full bg-red-600 hover:bg-red-700 text-white text-[9px] py-2.5 rounded-lg font-black uppercase flex items-center justify-center gap-2" onClick={() => navigateTo && navigateTo('form', { clientId: activeSite.id, clientName: activeSite.name })}>
                  Crear Inspección
                </button>
              </div>
            </div>
      ...
      {/* FILTRO DE INSPECCIONES (izquierda) - MANTENIDO */}
      ...
      <div className="absolute bottom-36 right-6 z-[1000]">
        <button onClick={() => userPos && map?.panTo(userPos)} className="bg-red-600 p-5 rounded-3xl border-4 border-red-400/30 text-white active:scale-90 shadow-2xl transition-all hover:bg-red-500"><LocateFixed size={28} /></button>
      </div>
