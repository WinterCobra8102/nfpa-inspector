...
const IPM_CATALOG = [
  {
    ...
    standard: "NFPA 25",
    category: "BOMBAS",
    name: "BOMBA INCENDIO DIESEL (MENSUAL)",
    formCode: "F-SER-014",
    icon: Activity,
    color: "#ef4444",
    hasVoltages: true,
    sections: [
      {
        ...
        points: [
          "Ejercitar cerrando y abriendo las válvulas normalmente abiertas del cuarto de bombas",
          "Ejercitar el indicador de nivel del tanque de almacenamiento de agua",
          "El tanque de almacenamiento de agua no cuenta con materiales extraños o deshechos",
          "Operar manualmente la válvula de llenado automático del tanque de almacenamiento de agua",
          "Ejercitar el indicador de nivel del tanque de combustible",
          "Probar el interruptor aislador del controlador jockey",
          "Activar la protección térmica del controlador jockey",
          "Ejercitar los interruptores del suministro de AC del controlador diesel",
          "Ejercitar los interruptores del suministro de baterías del controlador diesel",
          "Inspeccionar los componentes internos del controlador diesel",
          "Rellenar con agua destilada las celdas de las baterías con bajo nivel de electrolito",
          "Retirar corrosión de la batería y limpiar su carcasa",
          "Inspeccionar y realizar servicio de limpieza de filtros de la línea de suministro de agua",
          "Realizar limpieza general del cuarto de bombas en caso necesario",
        ],
      },
      {
        title: "PRUEBAS",
        points: [
          "Durante las pruebas las protecciones térmicas, disyuntores y fusibles operaron correctamente",
          "Realizar 6 arranques manuales alternando baterías 1 y 2",
  ...
  },
  {
    ...
    standard: "NFPA 25",
    category: "BOMBAS",
    name: "BOMBA INCENDIO DIESEL (SEMANAL)",
    formCode: "F-SER-015",
    icon: Activity,
    color: "#ef4444",
    sections: [
      {
        title: "INSPECCIÓN Y MANTENIMIENTO SEMANAL",
        points: [
          "Inspección visual de equipos operativos",
          "Verificar estado de bombas, tuberías y mangueras",
          "Revisar controlador de bomba contra incendio",
          "Verificar indicador de nivel de combustible",
          "Inspeccionar terminales de baterías",
          "Verificar precalentador de motor",
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        title: "MANTENIMIENTO",
        points: [
          "Servicio de limpieza a gabinete y rack de manguera",
  ...
  },
  {
    ...
    sections: [
      {
        title: "INSPECCIONES DEL PANEL",
        points: [
          "Tablero de control en buen estado y operativo",
          "Dispositivos manuales operativos",
          "Detectores de incendio en buen estado",
          "Fuentes de poder auxiliares operativas",
          "Baterías de respaldo en buen estado",
        ],
      },
      {
        title: "PRUEBAS DE SISTEMA",
        points: [
          "Prueba de luces del tablero",
          "Prueba de estaciones manuales",
          "Prueba de detectores de humo",
          "Prueba de notificación sonora y visual",
          "Verificar dispositivos de monitoreo",
        ],
      },
      {
    ...
    sections: [
      {
        title: "INSPECCIONES",
        points: [
          "El hidrante tiene libre acceso",
          "Las tapas giran libremente",
          "Verificar que el barril del hidrante esté libre de agua o hielo",
          "Estado físico del hidrante",
          "Desgaste de roscas en conectores de descarga y tapas",
          "Estado físico de la válvula",
    ...
    sections: [
      {
        title: "INSPECCIÓN",
        points: [
          "La válvula se encuentra operativa y libre de daño visible",
          "La válvula es accesible y libre de obstrucciones",
          "La válvula está equipada con la correspondiente llave para su manipulación",
  ...
  },
  {
    ...
    standard: "NFPA 25",
    category: "ROCIADORES",
    name: "SISTEMA DE ROCIADORES",
    formCode: "F-SER-IPM06",
    icon: Droplets,
    color: "#10b981",
    sections: [
      {
        title: "INSPECCIONES",
        points: [
          "Verificar que el sistema se encuentre operativo",
          "Anotar la presión de suministro del riser",
          "Anotar presión de agua o aire en el system",
          "Verificar fugas y daño físico en válvula de alarma o acción previa",
          "Verificar que las válvulas estén accesibles y en estado correcto",
          "Verificar placa de identificación del riser",
          "Verificar conexión con bomberos",
          "Verificar que las válvulas estén enclavadas o supervisadas",
          "Verificar que se cuenta con rociadores de repuesto",
        ],
      },
    ],
  },
  {
    ...
    name: "REPORTE DE OBSERVACIONES TÉCNICAS",
    formCode: "F-SER-045",
    icon: Clipboard,
    color: "#0f172a",
    isObservations: true,
    sections: [
      {
        title: "REPORTE",
        points: ["Revisión general de anomalías detectadas en sitio"],
      },
    ],
  },
];
...
export default function NewInspection({ navigateTo, prefillData }) {
  ...
  const [customAddModal, setCustomAddModal] = useState({
    isOpen: false,
    sIdx: null,
    isInventory: false,
    tag: "",
  ...
  const [customDeleteModal, setCustomDeleteModal] = useState({
    isOpen: false,
    sIdx: null,
    pIdx: null,
    pointValue: "",
      ...
      if (user) {
        ...
        if (!displayName) {
          try {
            ...
            console.warn("No se encontró el perfil en la base de datos");
          }
        }
    ...
    if (prefillData?.cliente_id && clientsDb.length > 0) {
      ...
      if (prefillData.normativa_nfpa) {
        const matchedIPM =
          IPM_CATALOG.find(
            (item) =>
              prefillData.normativa_nfpa
                .toUpperCase()
                .includes(item.standard.toUpperCase()) &&
              prefillData.normativa_nfpa
                .toUpperCase()
                .includes(item.category.toUpperCase()),
          ) ||
          IPM_CATALOG.find((item) =>
            prefillData.normativa_nfpa
              .toUpperCase()
              .includes(item.standard.toUpperCase()),
          );
  ...
  const confirmAddPoint = () => {
    ...
    if (isInventory) {
      if (!tag.trim() || !desc.trim())
        return toast.error("Ambos campos son requeridos");
    } else {
  ...
  const triggerRemovePoint = (sIdx, pIdx, pointValue) => {
    const displayName = pointValue.includes("|")
      ? pointValue.split("|")[1].trim()
      : pointValue;
  ...
  const handleSave = async () => {
    if (!selectedClient || !ownerName)
      return toast.error("Falta seleccionar cliente o responsable.");
    if (!clientSigData)
      return toast.error("La firma de conformidad del cliente es obligatoria.");
    if (!techSigData)
      return toast.error(
        "La firma de autorización del técnico es obligatoria.",
      );
    ...
    try {
      ...
      await db.inspections.add({
        ...
        signature: clientSigData,
        techSignature: techSigData,
    ...
    return (
      <div className="max-w-2xl mx-auto p-4 md:p-8 space-y-6">
        <button
          ...
          className="text-sm font-medium text-slate-500 dark:text-slate-400 flex items-center gap-2 hover:text-red-600 dark:hover:text-red-500 transition-colors"
        >
        ...
        <div className="bg-white dark:bg-slate-900 p-8 border border-slate-200 dark:border-slate-700 shadow-sm rounded-xl">
          ...
          {prefillData?.cliente_id && (
            <div className="mt-3 flex items-center gap-1.5 text-xs font-medium text-amber-600 dark:text-amber-500 bg-amber-50 dark:bg-amber-950/30 p-2.5 rounded-lg border border-amber-200 dark:border-amber-900/50">
              ...
              <span>
                Sucursal bloqueada. Asignación directa por Orden de Servicio.
              </span>
            </div>
          )}
        </div>
        ...
        <div
          className={`${!selectedClient ? "opacity-50 pointer-events-none" : ""} space-y-3`}
        >
          <h3 className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide pl-1">
            Seleccione Marco Normativo
          </h3>
          {["NFPA 25", "NFPA 72"].map((std) => (
            <button
              ...
              className="w-full flex justify-between items-center p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl hover:border-red-500 dark:hover:border-red-500 hover:shadow-md transition-all group"
            >
  ...
  if (step === 2) {
    const services = IPM_CATALOG.filter(
      (item) =>
        item.standard.includes(selectedStandard) || item.id === "IPM-07",
    );
      ...
      <div className="max-w-3xl mx-auto p-4 md:p-8 space-y-8">
        ...
        <div className="border-b border-slate-200 dark:border-slate-700 pb-4">
          <h2 className="text-2xl font-semibold text-slate-900 dark:text-white tracking-tight">
            Catálogo {selectedStandard}
          </h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Seleccione el formulario de inspección a ejecutar.
          </p>
        </div>
        {categories.map((cat) => (
          <div key={cat} className="space-y-3">
            <span className="text-xs font-medium tracking-wide text-slate-400 dark:text-slate-500 uppercase border-b border-slate-100 dark:border-slate-700 pb-1 block w-full">
              {cat}
            </span>
              ...
              {services
                ...
                .map((item) => (
                  <button
                    ...
                    className="bg-white dark:bg-slate-900 p-4 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm flex items-center justify-between hover:border-red-500 dark:hover:border-red-500 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all text-left group"
                  >
                    <div className="flex items-center gap-4">
                      <Icon
                        name={item.icon}
                        size={20}
                        className="text-slate-400 dark:text-slate-500 group-hover:text-red-600 dark:group-hover:text-red-500 transition-colors"
                      />
                      <div>
                        <h3 className="font-medium text-slate-800 dark:text-slate-200 text-sm leading-tight">
                          {item.name}
                        </h3>
                        <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                          {item.id} | {item.formCode}
                        </p>
                      </div>
                    </div>
                    <ChevronRight
                      size={18}
                      className="text-slate-300 dark:text-slate-600 group-hover:text-red-500"
                    />
                  </button>
                ))}
            </div>
    ...
    <div className="bg-slate-50 dark:bg-slate-950 min-h-screen pb-24 pt-4 md:pt-8 px-2 md:px-4">
      ...
      <div className="max-w-5xl mx-auto bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 shadow-2xl rounded-sm overflow-hidden">
        <div className="border-b-[3px] border-slate-800 dark:border-slate-600 p-6 md:p-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight leading-tight">
                {selectedIPM.name}
              </h1>
          ...
          <div className="text-left md:text-right shrink-0 border-l-4 border-red-600 pl-4">
            ...
            <p className="text-lg font-black text-slate-800 dark:text-slate-200">
              {selectedIPM.standard}
            </p>
            <p className="text-[10px] font-bold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 px-2 py-0.5 rounded inline-block mt-1">
              {selectedIPM.formCode}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 border-b border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs">
          <div className="p-4 border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-700">
            ...
            <p className="font-bold text-slate-800 dark:text-slate-200 uppercase leading-tight">
              {clientsDb.find((c) => c.id === selectedClient)?.nombre ||
                "NO ESPECIFICADO"}
            </p>
          </div>
        ...
        <div className="p-6 md:p-10 space-y-12">
            ...
            dynamicSections.map((sec, sIdx) => (
              ...
              >
                <div className="bg-slate-100 dark:bg-slate-800 px-5 py-3 border-b border-slate-300 dark:border-slate-600 flex justify-between items-center">
                  <h3 className="font-black text-slate-800 dark:text-slate-200 text-sm uppercase tracking-widest">
                    {sec.title}
                  </h3>
                ...
                <div className="divide-y divide-slate-200 dark:divide-slate-700">
                  {sec.points &&
                    sec.points.map((p, pIdx) => {
                      ...
                      const hasNote =
                        details[p]?.note && details[p].note.trim() !== "";
                      const hasPhoto = details[p]?.photo;

                      const isInventory = sec.isInventoryTable;
                      const itemName = isInventory
                        ? p.split("|")[0]?.trim()
                        : p;
                      const itemLocation = isInventory
                        ? p.split("|")[1]?.trim()
                        : null;
                      ...
                      return (
                        <div
                          key={pIdx}
                          className="flex flex-col hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors relative group"
                        >
                          <button
                            type="button"
                            ...
                            <div className="flex-1 py-4 pr-4 border-b lg:border-b-0 lg:border-r border-slate-100 dark:border-slate-700 flex flex-col justify-center">
                              {isInventory ? (
                                <div className="flex items-start gap-2">
                                  <span className="text-[9px] font-black text-slate-600 dark:text-slate-400 bg-slate-200 dark:bg-slate-700 px-1.5 py-0.5 rounded">
                                    {itemName}
                                  </span>
                            ...
                            <div className="flex items-center gap-2 p-3 shrink-0">
                              <button
                                ...
                                onClick={() =>
                                  setDetails((prev) => ({
                                    ...prev,
                                    [p]: { ...prev[p], status: "optimo" },
                                  }))
                                }
                                className={`w-9 h-9 rounded flex items-center justify-center border transition-all ${currentStatus === "optimo" ? "bg-green-500 text-white border-green-500 scale-110" : "border-slate-200 dark:border-slate-600 text-slate-400 dark:text-slate-500 hover:border-green-400 hover:text-green-500"}`}
                              >
                              ...
                              <button
                                ...
                                onClick={() =>
                                  setDetails((prev) => ({
                                    ...prev,
                                    [p]: { ...prev[p], status: "advertencia" },
                                  }))
                                }
                                className={`w-9 h-9 rounded flex items-center justify-center border transition-all ${currentStatus === "advertencia" ? "bg-amber-500 text-white border-amber-500 scale-110" : "border-slate-200 dark:border-slate-600 text-slate-400 dark:text-slate-500 hover:border-amber-400 hover:text-amber-500"}`}
                              >
                              ...
                              <button
                                ...
                                onClick={() =>
                                  setDetails((prev) => ({
                                    ...prev,
                                    [p]: { ...prev[p], status: "critico" },
                                  }))
                                }
                                className={`w-9 h-9 rounded flex items-center justify-center border transition-all ${currentStatus === "critico" ? "bg-red-500 text-white border-red-500 scale-110" : "border-slate-200 dark:border-slate-600 text-slate-400 dark:text-slate-500 hover:border-red-400 hover:text-red-500"}`}
                              >
                              ...
                              <button
                                ...
                                className={`w-9 h-9 rounded flex items-center justify-center border transition-all ${hasNote ? "bg-blue-500 text-white border-blue-500" : "border-slate-200 dark:border-slate-600 text-slate-400 dark:text-slate-500 hover:border-blue-400 hover:text-blue-500"}`}
                              >
                                <MessageSquare size={14} />
                              </button>
                              <label
                                className={`w-9 h-9 rounded flex items-center justify-center border transition-all cursor-pointer ${hasPhoto ? "bg-purple-500 text-white border-purple-500" : "border-slate-200 dark:border-slate-600 text-slate-400 dark:text-slate-500 hover:border-purple-400 hover:text-purple-500"}`}
                              >
                                <Camera size={14} />
                                <input
                                  type="file"
          ...
          {selectedIPM.hasVoltages && (
            <div className="border border-slate-300 dark:border-slate-600 rounded-sm overflow-hidden mb-10">
              <div className="bg-slate-100 dark:bg-slate-800 px-5 py-3 border-b border-slate-300 dark:border-slate-600 flex items-center justify-between">
                <h3 className="font-black text-slate-800 dark:text-slate-200 text-sm uppercase tracking-widest">
                  LECTURAS: 6 ARRANQUES MANUALES (VCD)
                </h3>
              </div>
              ...
              <div className="divide-y divide-slate-200 dark:divide-slate-700">
                {voltages.map((v, i) => (
                  <div
                    key={i}
                    className="grid grid-cols-3 divide-x divide-slate-200 dark:divide-slate-700 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
                  >
                    ...
                    <div className="py-2 px-2 flex items-center justify-center">
                      <input
                        type="number"
                        placeholder="0.0"
                        className="w-24 text-center bg-transparent border-b border-slate-300 dark:border-slate-600 focus:border-blue-500 text-sm font-bold outline-none text-slate-800 dark:text-slate-200 pb-1"
                        value={v.min}
                        onChange={(e) => {
                          const n = [...voltages];
          ...
          <div className="border border-slate-300 dark:border-slate-600 rounded-sm overflow-hidden mb-8">
            <div className="bg-slate-100 dark:bg-slate-800 px-5 py-3 border-b border-slate-300 dark:border-slate-600">
              <h3 className="font-black text-slate-800 dark:text-slate-200 text-sm uppercase tracking-widest">
                DICTAMEN TÉCNICO Y OBSERVACIONES GENERALES
              </h3>
            </div>
            <textarea
        ...
        <div className="border-t-[3px] border-slate-800 dark:border-slate-600 bg-slate-50 dark:bg-slate-800 p-6 md:p-10">
          <h3 className="text-center font-black text-slate-800 dark:text-slate-200 uppercase tracking-widest mb-12 text-sm">
            ...
            <div className="flex flex-col items-center w-full max-w-[320px] mx-auto">
              ...
              >
                {clientSigData ? (
                  ...
                  <span className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors mb-2">
                    Tocar para firmar
                  </span>
                )}
              </div>
              <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-3 text-center">
        ...
        <div className="fixed inset-0 z-[9999] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-lg shadow-xl overflow-hidden animate-in zoom-in-95 border border-slate-200 dark:border-slate-700">
            <div className="bg-slate-50 dark:bg-slate-800 p-4 flex justify-between items-center border-b border-slate-200 dark:border-slate-700">
              ...
              <button
                onClick={() =>
                  setCustomAddModal({ ...customAddModal, isOpen: false })
                }
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  autoFocus={!customAddModal.isInventory}
                  className="w-full p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 uppercase text-slate-800 dark:text-slate-200"
                  value={customAddModal.desc}
                  onChange={(e) =>
                    setCustomAddModal({
                      ...customAddModal,
                      desc: e.target.value,
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-800 flex gap-3 border-t border-slate-200 dark:border-slate-700">
              <button
                ...
                className="flex-1 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 rounded-lg font-medium text-sm transition-colors hover:bg-slate-100 dark:hover:bg-slate-700"
              >
                Cancelar
              </button>
              <button
                onClick={confirmAddPoint}
                className="flex-[2] py-3 bg-blue-600 text-white rounded-lg font-medium text-sm transition-colors hover:bg-blue-700"
              >
                Insertar
              </button>
            </div>
      ...
      {customDeleteModal.isOpen && (
        <div className="fixed inset-0 z-[9999] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-lg shadow-xl overflow-hidden animate-in zoom-in-95 text-center border border-slate-200 dark:border-slate-700">
            ...
            <div className="p-6">
              <h3 className="font-semibold text-slate-800 dark:text-slate-200 text-base mb-2">
                ¿Eliminar Fila?
              </h3>
              <div className="mt-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 text-left">
                <p className="font-medium text-slate-700 dark:text-slate-300 text-sm leading-snug">
                  {customDeleteModal.displayName}
                </p>
              </div>
            </div>
            <div className="p-4 bg-slate-50 dark:bg-slate-800 flex gap-3 border-t border-slate-200 dark:border-slate-700">
              <button
                onClick={() =>
                  setCustomDeleteModal({ ...customDeleteModal, isOpen: false })
                }
                className="flex-1 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 rounded-lg font-medium text-sm transition-colors hover:bg-slate-100 dark:hover:bg-slate-700"
              >
                Cancelar
              ...
              <button
                ...
                className="flex-1 py-3 bg-red-600 text-white rounded-lg font-medium text-sm transition-colors hover:bg-red-700"
              >
                Eliminar
              </button>
            </div>
      ...
      {showClientSigModal && (
        <div className="fixed inset-0 z-[9999] bg-slate-900 flex flex-col p-4 animate-in fade-in duration-200">
          <div className="flex justify-between items-center mb-4 mt-2">
            <div>
              ...
              <p className="text-slate-400 text-xs mt-0.5">
                {ownerName || "Sin responsable capturado"}
              </p>
            </div>
            <button
              onClick={() => setShowClientSigModal(false)}
          ...
          <div className="flex gap-4 mt-4 mb-2">
            <button
              onClick={() => clearSignature(clientCanvasRef)}
              className="flex-1 py-4 bg-slate-800 border border-slate-700 text-white rounded-lg font-medium text-sm transition-colors hover:bg-slate-700"
            >
              Limpiar
            </button>
            <button
              onClick={() =>
                saveSignature(
                  clientCanvasRef,
                  setClientSigData,
                  setShowClientSigModal,
                )
              }
              className="flex-[2] py-4 bg-blue-600 text-white rounded-lg font-medium text-sm transition-colors hover:bg-blue-500"
            >
              Guardar Firma
            </button>
          </div>
      ...
      {showTechSigModal && (
        <div className="fixed inset-0 z-[9999] bg-slate-900 flex flex-col p-4 animate-in fade-in duration-200">
          <div className="flex justify-between items-center mb-4 mt-2">
            ...
            <button
              onClick={() => setShowTechSigModal(false)}
              className="text-slate-400 hover:text-white transition-colors"
            >
              <X size={24} />
            </button>
          ...
          <div className="flex-1 w-full bg-white rounded overflow-hidden relative shadow-inner">
            ...
            <canvas
              ref={techCanvasRef}
              className="absolute inset-0 w-full h-full touch-none cursor-crosshair"
              onMouseDown={(e) => startDrawing(e, techCanvasRef)}
              onMouseMove={(e) => draw(e, techCanvasRef)}
              onMouseUp={() => setIsDrawing(false)}
              onMouseLeave={() => setIsDrawing(false)}
              onTouchStart={(e) => startDrawing(e, techCanvasRef)}
          ...
          <div className="flex gap-4 mt-4 mb-2">
            <button
              onClick={() => clearSignature(techCanvasRef)}
              className="flex-1 py-4 bg-slate-800 border border-slate-700 text-white rounded-lg font-medium text-sm transition-colors hover:bg-slate-700"
            >
              Limpiar
            </button>
      ...
      {imageToCrop && (
        <div className="fixed inset-0 z-[9999] bg-slate-900 flex flex-col p-4">
          <div className="relative flex-1 rounded overflow-hidden border border-slate-700">
            <Cropper
              image={imageToCrop}
              crop={crop}
              zoom={zoom}
          ...
          <button
            ...
            className="mt-4 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium flex items-center justify-center gap-2 transition-colors text-sm"
          >
            <Scissors size={18} /> Recortar y Anexar Imagen
          </button>
        </div>
      )}
    </div>
