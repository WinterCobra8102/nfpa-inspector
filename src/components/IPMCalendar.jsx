...
export default function IPMCalendar({ currentUser, navigateTo, selectedCompany, onBack }) {
  ...
  // ESTADO TEMPORAL PARA NUEVA ACTIVIDAD DESDE CERO
    ...
    <div className="max-w-7xl mx-auto p-4 space-y-6 animate-in fade-in duration-500 pb-32 relative overflow-hidden">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          {canAssign && (
            <button 
              ...
              className="flex items-center gap-2 bg-red-600 text-white px-5 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-red-700 transition-all active:scale-95 shadow-md"
            >
              <Plus size={16} /> Agregar Actividad
            </button>
          )}

          <button onClick={() => setShowGlossary(true)} className="flex items-center gap-2 bg-blue-50 text-blue-600 px-4 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all active:scale-95">
          ...
          <div className="flex gap-2 p-1.5 bg-slate-50 rounded-2xl border border-slate-100 items-center">
            ...
            {categories.map(cat => (
              <button key={cat.id} onClick={() => setFilterColor(cat.id)} className={`group relative w-8 h-8 rounded-full border-4 transition-all hover:scale-110 flex items-center justify-center ${cat.class} ${filterColor === cat.id ? 'border-slate-900 scale-110' : 'border-transparent opacity-40 hover:opacity-100'}`}>
      ...
      <div className="bg-slate-900 p-4 rounded-[1.5rem] flex flex-col lg:flex-row gap-4 items-center shadow-lg">
        <div className="w-full lg:w-1/3 bg-white/10 rounded-xl flex items-center px-4 border border-white/10 focus-within:border-red-500 transition-colors h-12">
          ...
          <input type="text" placeholder="Buscar válvula, bomba, hidrante..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-transparent text-white text-xs font-bold outline-none placeholder:text-slate-500" />
              ...
              {filteredTasks.filter(t => t.day_of_week === dia).map(task => (
                ...
                >
                  ...
                  {task.created_by_role === 'ADMIN' && (
                    <div className="absolute bottom-1 right-2 text-slate-300 group-hover:text-slate-400 transition-colors"><LockIcon size={10}/></div>
                  )}
          ...
          <div className="bg-white w-full max-w-xl rounded-[2.5rem] shadow-2xl relative z-10 overflow-hidden flex flex-col border-t-8 border-red-600 animate-in zoom-in-95 duration-150">
            <div className="p-6 bg-slate-50 border-b flex justify-between items-center">
              <div>
                ...
                <h3 className="font-black uppercase text-slate-800 text-lg mt-2">Nueva Actividad IPM</h3>
              </div>
            ...
            <div className="p-6 space-y-4 overflow-y-auto max-h-[60vh] text-slate-700">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-[9px] font-black uppercase text-slate-400 tracking-wider block mb-1">Normativa Relacionada</label>
              ...
              <div>
                ...
                <input placeholder="Ej: INSPECCIÓN DE VÁLVULAS..." className="w-full p-3 bg-slate-50 border rounded-xl font-bold text-xs outline-none focus:border-red-500 uppercase" value={newTask.title} onChange={e => setNewTask({...newTask, title: e.target.value})} />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-[9px] font-black uppercase text-slate-400 tracking-wider block mb-1">Frecuencia</label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-[9px] font-black uppercase text-slate-400 tracking-wider block mb-1">Período de Visita</label>
      ...
      {showGlossary && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl relative z-10 animate-in zoom-in-95 overflow-hidden flex flex-col max-h-[85vh]">
