...
export default function IPMCalendar({ currentUser, navigateTo }) {
  ...
  const [selectedTask, setSelectedTask] = useState(null); // Tarea clickeada
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 space-y-6 animate-in fade-in duration-500 pb-32 relative overflow-hidden">
      ...
      {/* PESTAÑAS DE QUINCENA Y PROGRESO */}
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white p-3 rounded-[2rem] border border-slate-100 shadow-sm">
        <div className="flex gap-2 bg-slate-100 p-1 rounded-[1.5rem]">
          <button onClick={() => setActiveWeek('Semana 1')} className={`px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeWeek === 'Semana 1' ? 'bg-white text-red-600 shadow-sm scale-100' : 'text-slate-400 hover:text-slate-700'}`}>Visita 1 - Semana 1</button>
          <button onClick={() => setActiveWeek('Semana 3')} className={`px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeWeek === 'Semana 3' ? 'bg-white text-red-600 shadow-sm scale-100' : 'text-slate-400 hover:text-slate-700'}`}>Visita 2 - Semana 3</button>
        </div>
      ...
      {/* CUERPO DEL CALENDARIO (GRILLA CLICKEABLE) */}
      ...
      {/* --- MODAL DE ORDEN DE TRABAJO (GESTIÓN DE TAREA) --- */}
      {selectedTask && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          <div className={`bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl relative z-10 animate-in zoom-in-95 overflow-hidden flex flex-col border-t-8 ${
