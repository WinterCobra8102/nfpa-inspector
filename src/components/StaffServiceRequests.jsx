...
export default function StaffServiceRequests({ currentUser, navigateTo }) {
    ...
    if (currentUser?.id) {
      ...
      const channel = supabase
        ...
        .on(
          ...
          (payload) => {
            if (
              payload.new.status === "ASIGNADO" &&
              payload.old.status === "PENDIENTE"
            ) {
  ...
  const handleStartWork = async (ticket) => {
    ...
    try {
      ...
      toast.success("Trabajo iniciado. Redirigiendo a la inspección...", {
        id: actionToast,
      });
      fetchMyRequests();
      ...
      setTimeout(() => {
        if (typeof navigateTo === "function") {
          ...
          toast.error(
            "Error interno: La función de navegación no está conectada.",
          );
        }
      }, 1000);
    } catch (err) {
  ...
  const confirmCompleteWork = async () => {
    ...
    try {
      ...
      toast.success("¡Orden completada y archivada exitosamente!", {
        id: actionToast,
      });
      ...
      <div className="space-y-4">
        {loading ? (
          <div className="p-10 text-center text-slate-400 dark:text-slate-500 text-sm flex items-center justify-center gap-2">
            <RefreshCw className="animate-spin" size={16} /> Sincronizando tus
            órdenes...
          </div>
        ) : requests.length === 0 ? (
          <div className="bg-white dark:bg-slate-900 p-12 rounded-xl border border-slate-200 dark:border-slate-700 text-center shadow-sm space-y-3">
            <CheckCircle2
              size={48}
              className="text-green-500 dark:text-green-400 mx-auto opacity-50"
            />
            <h3 className="text-lg font-medium text-slate-800 dark:text-slate-200">
              ¡Todo al día!
            </h3>
            <p className="text-sm text-slate-400 dark:text-slate-500">
              No tienes ninguna orden de servicio pendiente.
            </p>
          </div>
        ) : (
          requests.map((ticket) => (
            <div
              key={ticket.id}
              className="relative bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 hover:border-slate-300 dark:hover:border-slate-600 hover:shadow-md transition-all"
            >
              <div className="space-y-3 flex-1 pr-4">
                <div className="flex flex-wrap items-center gap-2">
                  <span
                    className={`text-[10px] font-bold px-2.5 py-1 rounded-md ${getStatusBadge(ticket.status)}`}
                  >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
                  ...
                  {ticket.fecha_programada && (
                    <div className="flex items-center gap-2 text-xs font-medium text-slate-700 dark:text-slate-200 bg-purple-50 dark:bg-purple-900/20 px-3 py-2 rounded-lg border border-purple-100 dark:border-purple-800 animate-pulse">
                      ...
                      <span>
                        Cita:{" "}
                        <span className="font-bold text-purple-700 dark:text-purple-400 ml-1">
                          {new Date(
                            ticket.fecha_programada + "T00:00:00",
                          ).toLocaleDateString()}{" "}
                          -{" "}
                          {ticket.hora_programada
                            ? formatTime(ticket.hora_programada)
                            : "Hora no definida"}
                        </span>
                      </span>
                    </div>
                ...
                {ticket.status === "COMPLETADO" ? (
                  ...
                  <>
                    ...
                    <div className="text-center space-y-1 mb-2">
                      <AlertCircle
                        size={20}
                        className="text-amber-500 mx-auto animate-pulse"
                      />
                    ...
                    <button
                      ...
                      disabled={isSubmitting}
                      onClick={() =>
                        triggerCompleteWork(ticket.id, ticket.titulo)
                      }
                    ...
                    >
                      {isSubmitting ? (
                        ...
                        <>
                          <CheckCheck size={18} /> Concluir Orden
                        </>
                      )}
                    </button>
                  </>
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 text-center border border-slate-200 dark:border-slate-700">
            ...
            <div className="p-6">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
                ¿Confirmas que has terminado satisfactoriamente la orden
                técnica?
              </p>
              <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-100 dark:border-slate-700">
                <p className="font-medium text-slate-700 dark:text-slate-300 text-sm italic">
                  "{concludeModal.title}"
                </p>
              </div>
            </div>
