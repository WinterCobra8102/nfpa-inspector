...
export default function ClientServiceRequests({ currentUser }) {
        ...
        .on(
          ...
          (payload) => {
            if (payload.new.status !== payload.old.status) {
              ...
              toast.success(
                `Tu solicitud ha cambiado de estado a: ${statusName}`,
                {
                  icon: "🔔",
                  duration: 5000,
                  position: "top-right",
  ...
  async function initializeClientData() {
    ...
    if (profile?.client_id) {
      ...
      setLoading(false);
      toast.error(
        "Tu perfil no tiene una empresa asignada. Contacta a soporte.",
      );
    }
  }
  ...
  const handleSubmitRequest = async (e) => {
    ...
    const actionToast = toast.loading(
      "Enviando solicitud al equipo de soporte...",
    );
    ...
    try {
      ...
      toast.success(
        "Solicitud enviada con éxito. En breve te asignaremos un técnico.",
        { id: actionToast },
      );
      setTitulo("");
      setDescripcion("");
    ...
    switch (status) {
      case "PENDIENTE":
        return {
          ...
          text: "En Revisión (Administración)",
          icon: <Clock size={14} />,
        };
      case "ASIGNADO":
        return {
          ...
          text: "Recibido - Técnico Asignado",
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
              Levantar reporte o solicitar servicio
            </p>
          </div>
        </div>
      </div>
          ...
          {loading ? (
            ...
            requests.map((ticket) => {
              ...
              return (
                <div
                  key={ticket.id}
                  className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col gap-3 hover:shadow-md transition-shadow duration-200"
                >
                  <div className="flex justify-between items-start gap-4">
                    ...
                    <div
                      className={`shrink-0 flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-md border ${statusData.style}`}
                    >
                      {statusData.icon}
                      {statusData.text}
                    </div>
