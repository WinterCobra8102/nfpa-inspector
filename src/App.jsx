...
function App() {
                    ...
                    <div className="flex items-start">
                      ...
                      <div className="ml-3 flex-1">
                        <p className="text-sm font-bold text-slate-900 dark:text-white">
                          Nuevo mensaje de{" "}
                          {senderProfile?.full_name || "Alguien"}
                        </p>
      ...
      try {
        ...
        if (!error && data) {
          ...
          try {
            ...
            const { data: remoteClientes, error: clientesError } =
              await clientesQuery;
            ...
            console.warn(
              "Offline: no se pudieron sincronizar clientes",
              syncErr,
            );
          }
        }
  ...
  const stats = {
    ...
    criticals:
      visibleInspections?.filter((i) => i.overallStatus === "CRÍTICO").length ||
      0,
    totalAssets: visibleInspections
      ? [...new Set(visibleInspections.map((i) => i.equipmentName))].length
      : 0,
  };
      ...
      <>
        ...
        <div className="h-screen w-screen bg-white dark:bg-slate-900 flex flex-col items-center justify-center relative overflow-hidden transition-colors duration-300">
          <div className="relative z-10 flex flex-col items-center">
            ...
            <h2 className="text-slate-900 dark:text-white font-semibold text-xl tracking-tight">
              TLETL
            </h2>
    ...
    return (
      <>
        ...
        <div className="flex h-screen flex-col items-center justify-center bg-white dark:bg-slate-900 p-6 text-center transition-colors duration-300">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 max-w-sm mt-3">
            El acceso a la plataforma ha sido restringido. Por favor, contacta
            con el administrador del sistema.
          </p>
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-8 border border-slate-200 dark:border-slate-800 px-4 py-2 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
            Código de Estatus: SUSP_SYS_2.0_SAAS
          </p>
          <button
            onClick={handleLogout}
            className="mt-10 text-sm font-medium text-slate-400 dark:text-slate-500 hover:text-red-600 dark:hover:text-red-500 transition-colors flex items-center gap-2"
          >
      ...
      <div className="flex h-screen bg-slate-50 dark:bg-slate-950 font-sans overflow-hidden transition-colors duration-300">
        ...
        >
          ...
          <div className="p-4 flex items-center justify-between shrink-0 border-b border-slate-100 dark:border-slate-800">
            <div className="flex items-center gap-3">
              ...
              {(isSidebarOpen || isMobileMenuOpen) && (
                <div className="flex flex-col">
                  <span className="font-semibold text-sm text-slate-900 dark:text-white tracking-tight leading-none">
                    TLETL
                  </span>
            ...
            <button
              onClick={() => setMobileMenuOpen(false)}
              className="md:hidden p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-400 dark:text-slate-500"
            >
              <X size={20} />
            </button>
          </div>
          ...
          <nav className="flex-1 mt-2 overflow-y-auto flex flex-col px-2">
            ...
            <NavItem
              ...
              label={
                ["MANAGER", "CLIENTE"].includes(currentUser?.role)
                  ? "Mi Sucursal (IPM)"
                  : "Empresas (IPM)"
              }
            ...
            {["ADMIN", "SUPER_ADMIN"].includes(currentUser.role) && (
              <>
                ...
                <NavItem
                  ...
                  label="Parámetros NFPA"
                  active={activeTab === "nfpa"}
                  onClick={() => navigateTo("nfpa")}
                  isOpen={isSidebarOpen || isMobileMenuOpen}
                />
              </>
            )}
          ...
          {/* Status bar */}
          <div
            className={`p-4 flex items-center gap-2.5 text-xs border-t border-slate-100 dark:border-slate-800 ${isOnline ? "text-green-600 dark:text-green-500" : "text-orange-500 dark:text-orange-400"}`}
          >
            ...
            {(isSidebarOpen || isMobileMenuOpen) && isOnline && (
        ...
        <main className="flex-1 flex flex-col min-w-0 relative h-full overflow-hidden">
          ...
          <header className="bg-white dark:bg-slate-900 h-14 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4 md:px-6 z-[2000] shrink-0 transition-colors duration-300">
            <div className="flex items-center gap-3">
              <button
                ...
                className="md:hidden p-2 -ml-2 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition-colors"
              >
                <Menu size={20} />
              </button>
              <button
                onClick={() => setSidebarOpen(!isSidebarOpen)}
                className="hidden md:flex p-2 -ml-2 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition-colors"
              >
                <Menu size={20} />
              </button>
            ...
            <div className="flex items-center gap-2 md:gap-4">
              ...
              <div className="flex items-center gap-3 pl-1">
                ...
                <button
                  ...
                  className="w-9 h-9 bg-slate-100 dark:bg-slate-800 rounded-lg flex items-center justify-center text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all border border-slate-200 dark:border-slate-700 overflow-hidden group p-0"
                >
          ...
          <div className="flex-1 overflow-y-auto bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
            ...
            {activeTab === "pump-calc" && (
              <PumpEfficiency currentUser={currentUser} />
            )}
            {(activeTab === "criticals" || activeTab === "critical") && (
              <CriticalFindings
                currentUser={currentUser}
            ...
            {activeTab === "tickets" &&
              (["ADMIN", "SUPER_ADMIN"].includes(currentUser.role) ? (
                <AdminServiceRequests currentUser={currentUser} />
...
function NavItem({ icon, label, active, onClick, isOpen }) {
  return (
    ...
    >
      <span
        className={`${active ? "text-red-600 dark:text-red-500" : "text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300"} transition-colors`}
      >
        {icon}
      </span>
      {isOpen && <span className="text-sm tracking-tight">{label}</span>}
      {active && isOpen && (
        <div className="ml-auto w-1.5 h-1.5 bg-red-600 dark:bg-red-500 rounded-full shadow-[0_0_8px_rgba(220,38,38,0.5)]"></div>
      )}
    </button>
  );
}
